...
class CollectiveBibliocustomviewsLayer(PloneSandboxLayer):

    defaultBases = (PLONE_FIXTURE, )
    """Layer to setup the bibliocustomviews site"""
    class Session(dict):
        def set(self, key, value):
            self[key] = value

    def setUpZope(self, app, configurationContext):
        """Set up the additional products required for the collective) site bibliocustomviews.
        ...
        # -----------------------------------------------------------------------
        # Load our own bibliocustomviews
        # -----------------------------------------------------------------------
        import collective.bibliocustomviews
        self.loadZCML('configure.zcml', package=collective.bibliocustomviews)
        ...
        z2.installProduct(app, 'collective.bibliocustomviews')
    ...
    def setUpPloneSite(self, portal):
        self.portal = portal
        applyProfile(portal, 'collective.bibliocustomviews:default')


class LayerMixin(object):
    defaultBases = (CollectiveBibliocustomviewsLayer() ,)
...
COLLECTIVE_BIBLIOCUSTOMVIEWS_FIXTURE             = CollectiveBibliocustomviewsLayer()
COLLECTIVE_BIBLIOCUSTOMVIEWS_INTEGRATION_TESTING = IntegrationTesting(name = "CollectiveBibliocustomviews:Integration")
COLLECTIVE_BIBLIOCUSTOMVIEWS_FUNCTIONAL_TESTING  = FunctionalTesting( name = "CollectiveBibliocustomviews:Functional")
COLLECTIVE_BIBLIOCUSTOMVIEWS_SELENIUM_TESTING    = FunctionalTesting(bases = (SELENIUM_TESTING, COLLECTIVE_BIBLIOCUSTOMVIEWS_FUNCTIONAL_TESTING,), name = "CollectiveBibliocustomviews:Selenium")
